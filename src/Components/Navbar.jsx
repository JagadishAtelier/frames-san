...
export default function Navbar({ onOpenModal }) {
  ...
  // If the navbar hides (due to scroll), also close the mobile menu
  useEffect(() => {
    if (!showNav) {
      setOpen(false);
    }
  ...
  const NavLink = ({ href, label, onClick }) => (
    <a href={href} onClick={onClick} className="group relative overflow-hidden inline-block">
      <div className="relative">
        <div className="transition-transform duration-300 group-hover:-translate-y-full uppercase">
          {label}
        </div>
        <div className="absolute top-full left-0 transition-transform duration-300 group-hover:-translate-y-full uppercase">
          {label}
        </div>
      </div>
    </a>
